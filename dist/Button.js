...
var _propTypes = interopRequireDefault$4(propTypes);
...
var _CircularProgress = interopRequireDefault$4(CircularProgress_1);
});
...
var LoadingIcon = styled(CircularProgress$2).withConfig({
  displayName: 'LoadingIcon',
  componentId: 'sc-1qikllj-0'
})(['width:24px !important;height:24px !important;color:', ' !important;'], function (props) {
  return props.color || '' + shared['periwinkle'];
});
...
var Btn = styled.button.withConfig({
  ...
  componentId: 'nzgghs-0'
})(['&:not(.btn-loading),&:not(.btn-loading) *{cursor:pointer;}&.btn{min-width:200px;width:100%;border:0;outline:none;background-color:', ';color:', ';&.small{height:48px;padding:12px 24px 14px 24px;border-radius:4px;}&.medium{height:80px;padding:24px 16px 24px 24px;}&.large{height:100px;padding:24px 16px 24px 24px;}&:hover:not(.btn-default):not(.btn-loading):not([disabled]){background-color:#43e4e4;}&:active:not(.btn-default):not(.btn-loading):not([disabled]){background-color:#2fbcbc;}}&.btn-default{color:', ';background-color:', ';}&:disabled{background-color:', ';&,& *{cursor:not-allowed;}}& .btn-inner__container{display:flex;flex-direction:row;justify-content:space-between;align-items:center;& label{font-size:16px;font-weight:500;font-style:normal;font-stretch:normal;line-height:1.5;letter-spacing:normal;margin-right:32px;flex:1;text-align:left;}}.btn-bottom{position:absolute;bottom:0;}'], shared['tealish'], shared['white'], shared['periwinkle'], shared['white'], shared['cloudy-blue']);

var Button = function (_React$Component) {
  inherits$1(Button, _React$Component);

  function Button() {
    classCallCheck$1(this, Button);

    var _this = possibleConstructorReturn$1(this, (Button.__proto__ || Object.getPrototypeOf(Button)).call(this));
  ...
  createClass$1(Button, [{
    ...
    value: function getIcon() {
      if (this.props.isLoading) {
        ...
        var isWhiteBackground = className && className.includes('btn-default');
        var color = isWhiteBackground ? shared['periwinkle'] : shared['white'];
        return React.createElement(LoadingIcon, { color: color });
      } else if (this.props.showIcon) {
        return React.createElement(ArrowForward$1, null);
      }
    }
  }, {
    key: 'render',
    value: function render() {
      ...
      var klass = 'btn ' + (className || '') + ' ' + (isLoading ? 'btn-loading' : '') + ' ' + (size || 'medium');
      return React.createElement(
        Btn,
        { type: type, className: klass, disabled: disabled, onClick: this.onClickHandler },
        React.createElement(
          'div',
